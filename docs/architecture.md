...
| `tmosc/api/routes/` | One `APIRouter` per area: `health` (`/`, `/api/health`, `/api/status`), `macros` (list, trigger, switch, editor CRUD, order), `midi` (owner heartbeat/release - the heartbeat may carry the TotalMix window title, from which the bridge adopts the workspace (#30) -, activity relay, bindings TSV), `knobs` (meters, group capture, duck, set/enable/param), `device` (state, sweep, physical table, Global status, identify, probe, picker), `config` (mappings / channel map / snapshot map reads, saves, uploads, init-from-example, reload), `ws` (`/ws`) |
